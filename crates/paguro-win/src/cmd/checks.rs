...
pub fn run_checks(ctx: &Ctx<'_>) -> Vec<Check> {
    ...
    v.push(if wsl {
        check(
            "wsl2",
            State::Ok,
            "WSL2 is installed: distributions are installed inside it.",
        )
    } else {
        with_fix(
            check(
                "wsl2",
                State::Fail,
                "WSL2 is missing; the installer runs inside it.",
            ),
            true,
    ...
    let free = api
        ...
        .and_then(|vs| {
            vs.into_iter()
                .find(|x| x.drive().is_some_and(|d| d.eq_ignore_ascii_case(&sys)))
        })
        .map(|x| x.free);
    v.push(match free {
        Some(f) if f >= SPACE_OK => check(
            "disk_space",
            State::Ok,
            format!("{} free on {sys}, enough for a Linux image.", gib(f)),
        ),
        Some(f) if f >= SPACE_MIN => with_fix(
            check(
                "disk_space",
                State::Warn,
                format!("{} free on {sys}: enough for a small image only.", gib(f)),
            ),
            false,
        ...
        Some(f) => with_fix(
            check(
                ...
                State::Fail,
                format!(
                    "Only {} free on {sys}; a Linux image needs at least {}.",
                    gib(f),
                    gib(SPACE_MIN)
                ),
            ),
        ...
        None => check(
            "disk_space",
            State::Warn,
            format!("The free space on {sys} could not be read."),
        ),
    });
    ...
    v.push(if api.tpm_present() {
        check(
            "tpm",
            State::Ok,
            "A TPM is present: Linux can unlock with it, like Windows.",
        )
    } else {
        check(
            "tpm",
            State::Warn,
            "No TPM: Linux unlocks with a passphrase only.",
        )
    });
    match secureboot::describe(ctx) {
        Ok((sb, ca)) => {
            v.push(if sb {
                check(
                    "secure_boot",
                    State::Ok,
                    "Secure Boot is on: every step of starting Linux is verified.",
                )
            } else {
                check(
                    "secure_boot",
                    State::Warn,
                    "Secure Boot is off: it works, but nothing verifies what starts.",
                )
            });
            v.push(match (sb, ca) {
        ...
        Err(e) => {
            v.push(check(
                "secure_boot",
                State::Warn,
                format!("Secure Boot state unknown: {}", e.message),
            ));
            v.push(check(
                "microsoft_uefi_ca",
                State::Warn,
                "Unknown (Secure Boot state unknown).",
            ));
        }
    }
    v.push(match fast_startup(ctx) {
        Some(false) => check(
            "fast_startup",
            State::Ok,
            "Fast Startup is off: Windows' drive is always closed cleanly.",
        ),
        Some(true) => with_fix(
            check(
                "fast_startup",
                State::Warn,
                "Fast Startup is on: Windows' drive stays half-open, so Linux can only read it.",
            ),
            true,
        ...
        None => check(
            "fast_startup",
            State::Warn,
            "The Fast Startup setting could not be read.",
        ),
    });
    v
}
...
pub fn fix(ctx: &Ctx<'_>, id: &str) -> CmdResult {
    if !IDS.contains(&id) {
        return Err(CmdError::new(
            Exit::Usage,
            format!("unknown check {id:?} (one of {})", IDS.join(", ")),
        ));
    }
    ...
    if before.state == State::Ok {
        return Ok(
            Report::new(json!({ "id": id, "changed": false, "check": before }))
                .line(format!("{id}: nothing to fix")),
        );
    }
    let fix = before.fix.clone().filter(|f| f.automatic).ok_or_else(|| {
        CmdError::refused(format!(
            ...
            before
                .fix
                .as_ref()
                .map_or("see the check's explanation", |f| f.instruction.as_str())
        ))
    })?;
    let (prog, args): (&str, Vec<String>) = match id {
        ...
        "bitlocker" => (
            ...
            vec![
                "-protectors".into(),
                "-enable".into(),
                ctx.api.system_drive(),
            ],
        ),
        _ => {
            return Err(CmdError::internal(format!(
                "{id}: automatic fix without a command"
            )));
        }
    };
    ...
    if ctx.dry_run {
        return Ok(
            Report::new(json!({ "id": id, "changed": false, "plan": plan }))
                .line(format!("would run: {}", fix.instruction)),
        );
    }
    ctx.need_admin()?;
    ...
    let mut r = Report::new(json!({ "id": id, "changed": true, "plan": plan, "check": after }))
        .line(format!("{id}: {}", fix.instruction));
    if id == "wsl2" {
