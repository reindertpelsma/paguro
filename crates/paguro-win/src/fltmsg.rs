...
impl Message {
    pub fn protect(volume: [u8; 16], file_id: [u8; 16], deny: u32) -> Self {
        Message {
            kind: PROTECT,
            volume,
            file_id,
    ...
    pub fn encode(&self) -> [u8; SIZE] {
        ...
        put(6, &self.kind.to_le_bytes());
        put(8, &self.volume);
        put(24, &self.file_id);
        put(40, &self.deny.to_le_bytes());
        put(44, &self.operation.to_le_bytes());
        put(48, &self.process_id.to_le_bytes());
        put(52, &self.status.to_le_bytes());
        b
    }
    ...
    pub fn decode_event(b: &[u8]) -> Option<Message> {
        ...
        if u32_at(0)? != MAGIC || u16_at(4)? != VERSION || u16_at(6)? != EVENT {
            return None;
        }
        Some(Message {
            kind: EVENT,
            volume: b.get(8..24)?.try_into().ok()?,
            file_id: b.get(24..40)?.try_into().ok()?,
            deny: u32_at(40)?,
            operation: u32_at(44)?,
            process_id: u32_at(48)?,
            status: u32_at(52)? as i32,
        })
    }
}
