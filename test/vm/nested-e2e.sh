...
wssh() { ${WSSH_TIMEOUT:+timeout "$WSSH_TIMEOUT"} ssh -i "$WIN_KEY" -p "$SSH_PORT" -o StrictHostKeyChecking=no \
            -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -o ConnectTimeout=120 -o ServerAliveInterval=30 \
            paguro@127.0.0.1 "$@"; }
# Nested, everything is slow: a command gets three tries.
wtry() { local i; for i in 1 2 3 4 5; do wssh "$@" && return 0; sleep 60; done; return 1; }
...
t0=$(date +%s)
shot() { l2 "paguro-vm qmp --socket /run/paguro/vm/qmp-test.sock '{\"execute\":\"screendump\",\"arguments\":{\"filename\":\"/share/$1\",\"format\":\"png\"}}'" >/dev/null 2>&1 || true; }
for i in $(seq 240); do
    sleep 10
...
out=$(wtry 'powershell -NoProfile -Command "Get-PnpDevice -PresentOnly | Where-Object FriendlyName -match ''VirtIO Socket|VirtIO Serial|VirtIO Ethernet'' | ForEach-Object { $_.Status + '' '' + $_.FriendlyName }"' 2>&1 | tr -d '\r') || true
echo "$out" >> "$VMWORK/nested-guest.txt"
expect "nested: vsock device" "$out" 'OK VirtIO Socket'
out=$(l2 "cat /share/nested-launch.log; /sbin/ip -n paguro -brief link; /sbin/ip -brief link | grep -c paguro0 || true") || true
...
wtry 'shutdown /s /t 0' >/dev/null 2>&1 ||
    l2 "paguro-vm qmp --socket /run/paguro/vm/qmp-test.sock '{\"execute\":\"system_powerdown\"}'" >/dev/null 2>&1 || true
for _ in $(seq 60); do l2 "test -e /run/paguro/vm/qemu.pid" >/dev/null 2>&1 || break; sleep 5; done
out=$(l2 "cat /share/nested-launch.log | tail -3; paguro-vm teardown --work /run/paguro/vm 2>&1; pgctl status") || true
echo "$out" >> "$VMWORK/nested-l2.txt"
